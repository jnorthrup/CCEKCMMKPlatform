...
class Element(val src: Series<Byte>) {
    val n = src.size
    ...
    fun buildTree(): Join<Cursor, FlatIndex> {
    ...
    fun scan0(): Join<Cursor, FlatIndex> {
    ...
    fun scanCbor(): Join<Cursor, FlatIndex> {
        ...
        fun parseItem(p: Int): Int {
            ...
            np = when (mt) {
                ...
                5 -> {
                    val (len, np1) = readLen(np, ai); var kp = np1
                    if (len < 0L) while (kp < n && (src[kp].toInt() and 0xFF) != 0xFF) { kp = parseItem(kp); kp = parseItem(kp) }
