    ...
    sourceSets {
        ...
        val jvmTest by getting {
            dependencies {
                ...
                implementation("org.jetbrains.kotlinx:kotlinx-coroutines-test:1.11.0-rc02")
                findProject(":libs:quic")?.let { implementation(it) }
                findProject(":libs:ngsctp")?.let { implementation(it) }
                findProject(":libs:htx-client")?.let { implementation(it) }
                findProject(":libs:miniduck")?.let { implementation(it) }
            }
        }
